...
\begin{abstract}
This document contains the proof of the property C=AB-BA, iff Trace(C) = 0
\end{abstract}
...
We have to find,
\begin{align}
 Trace(\vec{C}) = C_{11}+C_{22} = Trace(\vec{AB}-\vec{BA})\\
 \implies  Trace(\vec{C}) = Trace(\vec{AB})-Trace(\vec{BA})\label{s}
\end{align}
We know that,
\begin{align}
Trace(\vec{AB}) = \sum_{i=1}^2(\vec{AB})_{ii}\\
\implies \sum_{i=1}^2\sum_{j=1}^2a_{ij}b_{ji}\\
\implies \sum_{j=1}^2\sum_{i=1}^2b_{ji}a_{ij}\\
\implies Trace(\vec{AB}) = \sum_{j=1}^2\vec{BA}_{jj}\\
\implies Trace(\vec{AB}) = Trace(\vec{BA})
\label{t}
\end{align}
Substituting equation \eqref{t} to \eqref{s} we getm
\begin{align}
\implies  Trace(\vec{C}) = Trace(\vec{AB})-Trace(\vec{BA}) = 0
\end{align}
Hence, Proved
